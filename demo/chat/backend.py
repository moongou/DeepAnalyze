...
- **重要：使用 <TaskTree> 标签输出分析计划。标签内部必须是且仅是一个合法的 JSON 对象，不得包含任何其他文字、说明、代码块标记或换行文字。**
...
- 输出 <TaskTree> 标签后，**立即停止所有输出**。前端会自动弹出任务选择面板供用户勾选，你不需要再输出任何提示文字（不要输出"请选择…"等文字）。
- 用户确认后，系统会自动回复："用户选择了以下分析任务：[1] xxx, [1.1] xxx"。
