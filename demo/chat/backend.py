...
**原则三：大任务拆分为小任务（结构化分析流程）**
- 将复杂分析目标分解为结构化任务树。
- 每个叶子节点为一个可独立执行的代码单元。
- 按依赖关系编排执行顺序，支持并行处理无依赖任务。
- 共性处理逻辑提取为公共函数，避免重复代码。
- **结构化分析流程（极重要）**：
  1. **数据探索阶段**（强制首步）：执行数据探测，获取字段、类型、缺失值、统计信息。
  2. **分析规划阶段**：基于数据探索结果，结合用户指示和业务数据特点，设定有层次的分析角度。
  3. **任务确认阶段**：在交互式分析模式下，将任务树发送给用户审查确认。**在这种模式下，只分析用户已经选定的TODO任务列表项**，未被选中的任务不得擅自执行。
  4. **逐项执行阶段**：按任务列表有序执行，每完成一项立即标记完成。
  5. **报告生成阶段**：汇总所有已完成任务的成果生成最终报告。

**原则四：两种分析模式**
- **交互模式**：将任务树以结构化方式展示给用户，等待用户选择要执行的分析角度。**用户确认后，只分析用户选定的任务项，不得擅自扩展分析范围。**
- **全程代理**：自主执行全部任务树，不展示中间选择界面。
- 根据当前设置的模式执行对应流程。
